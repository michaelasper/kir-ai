...
use super::{
    F16Buffer, F32Buffer, F32TransientBuffer, I8Buffer, MetalDevice, MetalError,
    metal_buffer_byte_len, power_of_two_at_most,
};
use metal::MTLSize;
...
impl MetalDevice {
    ...
    pub async fn full_attention_cache_mix_f32_buffered_to_buffer(
        &self,
        keys: &F32Buffer,
        values: &F32Buffer,
        query: &[f32],
        ...
        if row_count == 0 {
            let output_buffer =
                self.take_scratch_f32_transient_buffer_len(attention_dim, "attention output")?;
            self.zero_f32_transient_buffer(&output_buffer)?;
        ...
        let output_buffer =
            self.take_scratch_f32_transient_buffer_len(attention_dim, "attention output")?;
        let Some(output_metal_buffer) = output_buffer.buffer.as_ref() else {
            return Err(MetalError::InvalidShape(
                "non-empty attention requires an output buffer".to_owned(),
            ));
        };
        ...
        sum_encoder.set_buffer(7, Some(output_metal_buffer), 0);
        ...
        finish_command_buffer_async(
            &[&keys.hazard, &values.hazard, &output_buffer.hazard],
            command_buffer,
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn full_attention_cache_mix_f16_buffered_to_buffer(
        &self,
        keys: &F16Buffer,
        values: &F16Buffer,
        query: &[f32],
        ...
        self.full_attention_cache_mix_f16_buffered_at_to_buffer(
            keys,
            0,
            values,
            0,
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn full_attention_cache_mix_f16_buffered_at_to_buffer(
        &self,
        keys: &F16Buffer,
    ...
    pub async fn select_head_rows_f32_buffered(
        ...
        encoder.dispatch_threads(threads, threads_per_group);
        encoder.end_encoding();
        finish_command_buffer_async(&[&values.hazard], command_buffer, "select_head_rows_f32")
            .await?;
    ...
    pub async fn select_head_rows_f16_buffered(
        ...
        encoder.dispatch_threads(threads, threads_per_group);
        encoder.end_encoding();
        finish_command_buffer_async(&[&values.hazard], command_buffer, "select_head_rows_f16")
            .await?;
