use llm_hub::{
    DeletedSnapshot, ModelLifecyclePlanOptions, ModelLifecycleRequest, ModelLifecycleService,
    ModelStore, PromotedSnapshot, ProtectedSnapshot, PruneCandidate, PrunePlan, PrunePolicy,
    PruneReport, QuarantinedSnapshot, SnapshotReadinessMode, SnapshotRecord,
};
...
pub async fn run(args: Vec<String>) -> anyhow::Result<()> {
    ...
    match subcommand.as_str() {
        "list" => {
            ...
            let readiness_mode = snapshot_readiness_mode_from_args(&args)?;
            let value = model_list_json_with_mode(&root, readiness_mode).await?;
        ...
        "inspect" => {
            ...
            let value = model_inspect_json(snapshot_path).await?;
            println!("{}", serde_json::to_string_pretty(&value)?);
        }
        ...
        "verify" => {
            ...
            let value = model_verify_json(snapshot_path).await?;
            println!("{}", serde_json::to_string_pretty(&value)?);
        }
        "prune" => {
            let mode = prune_mode_from_args(&args)?;
            let root = model_home_from_args(&args);
            let policy = prune_policy_from_args(&args)?;
            let value = model_prune_json(&root, policy, mode).await?;
            println!("{}", serde_json::to_string_pretty(&value)?);
        }
        #[cfg(feature = "diagnostics")]
        "inspect-safetensors" | "inspect-tensor" | "inspect-qwen-input" => {
            super::native_probe::run(subcommand, &args).await?;
        }
        #[cfg(not(feature = "diagnostics"))]
        "inspect-safetensors" | "inspect-tensor" | "inspect-qwen-input" => {
            diagnostics_feature_required(subcommand)?
        }
        "plan" | "pull" => {
            let request = model_lifecycle_request_from_args(subcommand, &args)?;
            let token = std::env::var("HF_TOKEN").ok();
            let hub_endpoint = flag_value(&args, "--hub-endpoint")
                .map(str::to_owned)
                .or_else(|| std::env::var("LLM_HUB_ENDPOINT").ok());
            let client = crate::configured_hub_client(hub_endpoint.as_deref(), token.as_deref())?;
...
pub fn model_home_from_args(args: &[String]) -> PathBuf {
    flag_value(args, "--model-home")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("LLM_MODEL_HOME").map(PathBuf::from))
