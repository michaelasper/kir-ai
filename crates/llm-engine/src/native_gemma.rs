...
impl NativeTextAdapter for NativeGemmaAdapter {
    ...
    fn cache_token_capacity(
        ...
        native_gemma_cache_token_capacity(
            context_tokens,
            max_new_tokens,
    ...
    fn next_token_from_hidden(
        ...
        let final_norm = gemma_final_norm_for_spec(&self.store, &self.spec, hidden)
            .map_err(|err| BackendError::Other(err.to_string()))?;
        if !sampling.is_greedy() {
            let logits = gemma_lm_head_logits_for_spec_with_matvec(
                &self.store,
                &self.spec,
                &final_norm,
                self.chunk_rows,
            ...
            u32::try_from(sampled_token_id).map_err(|err| {
                BackendError::Other(format!("Gemma token id does not fit u32: {err}"))
            })?;
            return Ok(sampled_token_id);
        }

        let top_k = self.top_k.min(self.spec.vocab_size as usize).max(1);
        let top_logits = gemma_lm_head_top_k_for_spec_with_matvec(
            &self.store,
            &self.spec,
            &final_norm,
            top_k,
        ...
        let item = top_logits
            .into_iter()
            .next()
            .ok_or_else(|| BackendError::Other("Gemma lm head returned no logits".to_owned()))?;
        u32::try_from(item.index).map_err(|err| {
            BackendError::Other(format!("Gemma token id does not fit u32: {err}"))
        })?;
        Ok(item.index)
    }
...
fn native_gemma_cache_token_capacity(
    ...
    let max_position_embeddings = usize::try_from(max_position_embeddings).map_err(|err| {
        BackendError::Other(format!(
            "native Gemma max_position_embeddings does not fit usize: {err}"
        ))
    })?;
    if max_position_embeddings == 0 {
        return Err(BackendError::UnsupportedRequest(
            "native Gemma model declares zero max_position_embeddings".to_owned(),
        ));
    }
    let max_new_tokens = usize::try_from(max_new_tokens).map_err(|err| {
        BackendError::Other(format!(
            "native Gemma max_new_tokens does not fit usize: {err}"
        ))
    })?;
    let requested_context = context_tokens.checked_add(max_new_tokens).ok_or_else(|| {
        BackendError::UnsupportedRequest(
            "native Gemma context length plus generation budget overflows usize".to_owned(),
        )
    })?;
    if requested_context > max_position_embeddings {
        return Err(BackendError::UnsupportedRequest(format!(
            "native Gemma request needs {context_tokens} prompt tokens plus {max_new_tokens} generation tokens, exceeding model context limit {max_position_embeddings}"
        )));
    }
...
fn sample_token_id_with_draw(
    ...
    native_text_sample_token_id_with_draw(logits, sampling, draw, "Gemma")
}

static NATIVE_GEMMA_SAMPLING_COUNTER: AtomicU64 = AtomicU64::new(0);
...
fn native_sampling_draw() -> f32 {
    let time_seed = SystemTime::now()
        ...
        .unwrap_or(0);
    let counter = NATIVE_GEMMA_SAMPLING_COUNTER.fetch_add(1, Ordering::Relaxed);
