...
- The native Qwen executor now covers embedding lookup, centered RMSNorm, hybrid linear/full attention first-token paths, full-attention q/k RMSNorm, q/k score dot products, softmax, and value mixing, linear-attention q/k/head normalization, routed MoE expert slices, MoE router top-k/softmax selection, selected/shared MoE accumulation, layer input/post-attention RMSNorm, final norm, streamed lm-head logits, and tokenizer decode for the first token with no Python or external inference engine.
...
- Native Qwen serving now routes bounded prefill, decode, MoE dense projections, router top-k/softmax selection, and selected/shared expert accumulation, full-attention q/k RMSNorm, q/k score dot products, softmax, and value mixing, linear-attention convolution+silu, q/k/head normalization, and recurrent memory/core dot products, layer input/post-attention RMSNorm, final RMSNorm, and lm-head matvecs through a configurable executor. The production executor uses Metal BF16/f32 matvec, Qwen RMSNorm, softmax, linear convolution, weighted-sum, and top-k kernels when a Metal device is available, with CPU fallback for unsupported shapes or non-Metal hosts.
...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable executor, but attention cache row gathering, recurrent-state mutation, MoE expert dispatch, and remaining control flow are still CPU-owned. The remaining Qwen Metal kernels are not complete.
...
- Direct Metal smoke compute, a Qwen RMSNorm kernel, `f32` softmax, `f32` weighted sum, linear-attention convolution+silu, row-major `f32` matvec, row-major BF16-weight matvec, batched BF16-weight matvec, and `f32` argmax/top-k logits selection are implemented. Native serving now calls full-attention q/k RMSNorm, q/k score dot products, softmax, and value mixing, linear-attention convolution+silu, q/k/head normalization plus recurrent memory/core dot products, MoE router top-k/softmax selection, selected/shared MoE accumulation, layer input/post-attention RMSNorm, final RMSNorm, and the matvec/top-k subset through a CPU-fallback executor; the remaining Qwen kernels are not complete.
