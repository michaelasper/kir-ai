...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable executor, and selected MoE expert gate/up/down slices now route through BF16 range matvecs. Attention cache storage/lifetime, recurrent-state cache storage, shared-expert f32 materialization, expert-selection control flow, and remaining control flow are still CPU-owned. The remaining Qwen Metal kernels are not complete.
