...
- `c0d8a74` - Metal includes a one-centered RMSNorm kernel covered against the CPU reference.
...
- `d42c045` - Native Qwen serving exposes `--warm-native-metal-weight-cache` to preload rank-2 BF16 tensors into the Metal weight-buffer cache at startup until the configured budget is full.
- `925e357` - Native text Metal weight-cache warm-up orders rank-2 BF16 tensors by model execution order instead of lexicographic tensor name order.
- `1b2f806` - Native Qwen Metal weight-cache state is shared process-locally by canonical snapshot namespace and cache budget, avoiding cross-model tensor-name collisions.
- `4d6c382` - Native Qwen MoE selected expert gate/up/down slices route through a BF16 range-matvec hook, enabling Metal cached tensor-slice execution.
- `1c1e530` - Native Qwen MoE shared expert gate/up/down and shared-expert gate projections route through BF16 matvec hooks.

Current verified state:

- `mise run fmt-check`, `mise run test`, and `mise run clippy` pass for the workspace.
- `mise run gates-ci` runs the fast north-star protocol/API, runtime agentic, HTTP, model CLI, model acquisition, model-family backend profile, deferred-family, tokenizer, and tool-parser gates. `mise run gates-nightly` adds no-progress replay, backend, Metal smoke, long-context dry-run, and configured 135K/200K long-context inference gates. Both profiles emit reproducible JSON and Markdown reports plus per-gate logs under `target/north-star-gates/`; the GitHub workflows upload those reports as artifacts and append the Markdown summary to the job summary.
- Model-family boundaries are explicit: Qwen is the first proving family, while generic backend cache identity uses family-provided chat-template IDs rather than Qwen-named backend methods. `llm-models` declares Qwen production backends as both native Metal and MLX, and `model_family_backend_profiles` is a required north-star CI gate. DeepSeek is a first-class MLX family with text/reasoning/tool/DSML/raw-completion capability flags; the runtime delegates MLX chat templating to the sidecar and only routes the DeepSeek parser names whose tool grammar is implemented. Gemma is a text-only first-class MLX family with reasoning-channel capability flags, text/tool/channel/multimodal fixtures, and a text-only acquisition profile that excludes vision/projector artifacts.
...
- HTTP generation admission now goes through a bounded model scheduler instead of a bare semaphore. The scheduler classifies prefill-heavy versus decode-interactive requests, exposes queued/active prefill/decode metrics plus scheduler completion/cancellation/failure counters, keeps stable `model_overloaded` errors when the queue is full or times out, and prioritizes decode work after a bounded prefill burst.
- Chat streaming now validates supported family-specific tool-call boundaries incrementally: complete Qwen, DeepSeek native/DSML, and Gemma tool-call blocks are parsed, checked against declared tools, and emitted as tool-call deltas before backend completion. JSON-object streaming starts as SSE but withholds assistant JSON content until final object validation, so invalid JSON returns stable SSE error metadata plus one terminal `[DONE]` without leaking invalid content as a successful delta.
...
- `llm-engine model prune --dry-run --model-home <path>` and `llm-engine model prune --confirm-delete --model-home <path>` share the same retention planner. The policy protects recorded active aliases, recently used snapshots, and the newest configured count per profile; destructive prune verifies candidates before deletion and quarantines corrupt candidates instead of deleting them. `model list`, `model inspect`, and `/admin/metrics` expose quarantined snapshots and bytes.
...
- `/admin/*` routes require `Authorization: Bearer <token>`. `--admin-token` or `LLM_ENGINE_ADMIN_TOKEN` sets a stable token; loopback binds without one generate a temporary process token at startup, and `serve --addr` refuses non-loopback binds unless an admin token is configured.
- `POST /admin/models/{alias}/plan` returns a native Rust download plan for the served model alias, and `POST /admin/models/{alias}/pull` pulls and promotes a snapshot through `ModelStore`. Both routes use the same admin Bearer-token enforcement; `serve` exposes `--model-home`, `--hub-endpoint`, and `HF_TOKEN` wiring for these operations.
...
- `llm-engine serve` no longer silently starts the deterministic/protocol backend when no snapshot is provided. Production serving requires `--snapshot <path>`, while protocol testing is explicitly gated behind `--protocol-test-backend`.
- `llm-metal` now includes a one-centered RMSNorm Metal kernel with smoke coverage against the CPU reference, in addition to the direct vector-add compute smoke.
...
- Native Qwen serving now routes bounded prefill, decode, MoE dense projections, selected expert BF16 gate/up/down slices, shared expert BF16 projections, router top-k/softmax selection, and selected/shared expert accumulation, full-attention q/k RMSNorm, cache key/value row gathering, q/k score dot products, softmax, and value mixing, linear-attention convolution+silu, q/k/head normalization, recurrent memory/core dot products, recurrent-state decay, and recurrent-state updates, layer input/post-attention RMSNorm, final RMSNorm, and lm-head matvecs through a configurable executor. The production executor uses Metal BF16/f32 matvec, reusable BF16 matrix buffers, one-centered RMSNorm, softmax, linear convolution, weighted-sum, recurrent-update, head-row selection, and top-k kernels when a Metal device is available, with CPU fallback for unsupported shapes or non-Metal hosts.
...
- Cache-backed full-attention single-token decode now also supports local sliding-window execution when the `LayerKvCache` is already full, evicting the oldest retained key/value row while keeping RoPE positions based on total tokens seen.
...
- Native Qwen Metal BF16 matvecs now upload each tensor slice into a reusable Metal matrix buffer on first use, share that buffer across single, batched, chunked, and lm-head top-k matvec calls within the same canonical snapshot namespace and cache budget, enforce a configurable default 8 GiB LRU budget through `--native-metal-weight-cache-bytes`, skip caching slices larger than the budget, report cache hits, misses, uploads, uploaded bytes, evictions, evicted bytes, resident bytes, resident buffer count, and active budget bytes, and can opt in to startup preloading of rank-2 BF16 tensors through `--warm-native-metal-weight-cache`. Warm-up uses model execution ordering so embeddings and earlier-layer attention/MoE projections are considered before later layers, `lm_head`, and unknown tensors.
