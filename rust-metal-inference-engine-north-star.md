...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable executor, but attention control flow, recurrent state updates, expert routing, and layer/internal normalization work are still CPU-owned. The remaining Qwen Metal kernels are not complete.
...
- Direct Metal smoke compute, a Qwen RMSNorm kernel, row-major `f32` matvec, row-major BF16-weight matvec, batched BF16-weight matvec, and `f32` argmax/top-k logits selection are implemented. Native serving now calls the final-RMSNorm and matvec/top-k subset through a CPU-fallback executor; the remaining Qwen kernels are not complete.
