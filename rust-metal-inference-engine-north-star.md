...
- Native Qwen serving now routes bounded prefill, decode, MoE dense projections, and lm-head matvecs through a configurable matvec executor. The production executor uses Metal BF16/f32 matvec and top-k kernels when a Metal device is available, with CPU fallback for unsupported shapes or non-Metal hosts.
...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable matvec executor, but attention control flow, recurrent state updates, expert routing, and some normalization work are still CPU-owned. The remaining Qwen Metal kernels are not complete.
