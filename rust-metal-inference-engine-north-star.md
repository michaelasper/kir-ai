...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable executor, but attention cache reads, recurrent-state mutation, convolution mixing, expert routing, and remaining control flow are still CPU-owned. The remaining Qwen Metal kernels are not complete.
