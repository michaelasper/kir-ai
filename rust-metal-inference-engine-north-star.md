...
- Large projection reads can now use Metal chunked BF16 matvecs with backend-owned reusable BF16 matrix buffers, a configurable per-backend LRU residency budget, residency metrics, and an opt-in startup warm plan when a device is available. GPU weight residency is still per backend process and lacks a model-wide/shared residency manager plus family-aware warm ordering. The current full 40-layer plus lm-head probe is correctness evidence, not yet the final persistent GPU-resident serving-performance path.
