...
- The native Qwen executor now covers embedding lookup, centered RMSNorm, hybrid linear/full attention first-token paths, routed MoE expert slices, layer input/post-attention RMSNorm, final norm, streamed lm-head logits, and tokenizer decode for the first token with no Python or external inference engine.
...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable executor, but attention control flow, recurrent state updates, expert routing, attention q/k normalization, and linear-attention internal/head normalization work are still CPU-owned. The remaining Qwen Metal kernels are not complete.
...
- Direct Metal smoke compute, a Qwen RMSNorm kernel, row-major `f32` matvec, row-major BF16-weight matvec, batched BF16-weight matvec, and `f32` argmax/top-k logits selection are implemented. Native serving now calls layer input/post-attention RMSNorm, final RMSNorm, and the matvec/top-k subset through a CPU-fallback executor; the remaining Qwen kernels are not complete.
