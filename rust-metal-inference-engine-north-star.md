...
- Native Qwen multi-token decode is wired through backend caches and a Metal-capable executor, but attention cache row gathering, recurrent-state decay/cache storage, MoE expert dispatch, and remaining control flow are still CPU-owned. The remaining Qwen Metal kernels are not complete.
...
- Direct Metal smoke compute, a Qwen RMSNorm kernel, `f32` softmax, `f32` weighted sum, linear-attention convolution+silu, linear-attention recurrent update, row-major `f32` matvec, row-major BF16-weight matvec, batched BF16-weight matvec, and `f32` argmax/top-k logits selection are implemented. Native serving now calls full-attention q/k RMSNorm, q/k score dot products, softmax, and value mixing, linear-attention convolution+silu, q/k/head normalization plus recurrent memory/core dot products and state updates, MoE router top-k/softmax selection, selected/shared MoE accumulation, layer input/post-attention RMSNorm, final RMSNorm, and the matvec/top-k subset through a CPU-fallback executor; the remaining Qwen kernels are not complete.
