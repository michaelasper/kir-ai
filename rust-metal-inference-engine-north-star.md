...
- Model-family boundaries are explicit: Qwen is the first proving family, while generic backend cache identity uses family-provided chat-template IDs rather than Qwen-named backend methods. `llm-models` declares Qwen production backends as both native Metal and MLX, and `model_family_backend_profiles` is a required north-star CI gate. DeepSeek is now a deferred first-class family with text/reasoning/tool/DSML/raw-completion capability flags, MLX as its declared production backend target, and fail-closed tokenizer/parser selectors covered by golden fixtures until Qwen production gates are green. Gemma is also deferred as a text-only first-class family with reasoning-channel capability flags, fail-closed template/parser selectors, text/tool/channel/multimodal fixtures, and a text-only acquisition profile that excludes vision/projector artifacts.
