...
run_ci_gates() {
  ...
  run_gate "runtime_agentic_contracts" true cargo test -p llm-runtime --test runtime_contract
  ...
  run_gate "deferred_family_contracts" true bash -lc 'cargo test -p llm-tokenizer --test deepseek_template && cargo test -p llm-tool-parser --test deepseek_parser'
